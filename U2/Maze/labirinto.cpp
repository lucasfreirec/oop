...
double Labirinto::calculaCaminho(int& prof, int& NAbert, int& NFech)
{
  ...
  Noh atual(getOrig(), getOrig(), 0.0), suc;

  atual.heuristica(getDest());
  ...
  do
  {
    ...
    {
        mov_ant = atual.getPos() - atual.getAnt();
        ...
        for(mov.lin = -1; mov.lin <= 1; ++mov.lin)
        {
            for(mov.col = -1; mov.col <= 1; ++mov.col)
            {
                if(mov != Coord(0,0))
                {
                    prox = atual.getPos() + mov;

                    if(movimentoValido(atual.getPos(), prox))
                    {
                        suc.setPos(prox);
                        suc.setAnt(atual.getPos());

                        if (mov_ant != Coord(0,0)) th = acos((mov * mov_ant)/(mov.modulo() * mov_ant.modulo()));

                        else th = 0;

                        suc.setG(atual.getG() + (th/2) + mov.modulo());

                        suc.heuristica(getDest());
                        ...
                        for(mov_viz.lin = -1; mov_viz.lin <= 1; ++mov_viz.lin)
                        {
                            for(mov_viz.col = -1; mov_viz.col <= 1; ++mov_viz.col)
                            {
                                ...
                                {
                                    viz = suc.getPos() + mov_viz;

                                    if (!celulaValidaLivre(viz)) suc.setG(suc.getG() + 0.0001);
                                }
                            }
                        }
                        ...
                        itr = find(Aberto.begin(), Aberto.end(), suc.pos);

                        if (itr != Aberto.end())
                        {
                           if (suc.getF() < itr->getF()) Aberto.erase(itr);
                        ...
                        {
                            itr = find(Fechado.begin(), Fechado.end(), suc.pos);

                             if (itr != Fechado.end())
                             {
                                if (suc.getF() < itr->getF()) Fechado.erase(itr);
                        ...
                        if (eh_inedito)
                        {
                            itr = upper_bound(Aberto.begin(), Aberto.end(), suc);
  ...
  while(!Aberto.empty() && atual.getPos() != getDest());
  ...
  else
  {
    ...
    while(atual.getAnt() != getOrig() && prof >= 0)
    {
        set(atual.getAnt(), EstadoCel::CAMINHO);

        itr = find(Fechado.begin(), Fechado.end(), atual.ant);
