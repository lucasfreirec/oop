...
void MainCircuito::slotNewCircuito(int NInputs, int NOutputs, int NPortas)
{
  ...
  C.resize(NInputs, NOutputs, NPortas); //FEITO
                                      ...
                                      int IdInput0, int IdInput1, int IdInput2, int IdInput3)
{
  ...
  C.setPort(IdPort, TipoPort.toStdString(), NumInputsPort); // FEITO
  ...
  if (NumInputsPort >= 2) C.setId_inPort(IdPort, 1, IdInput1); // FEITO
  // O parametro IdInput2 soh eh levado em conta se NumInputsPort>=3
  if (NumInputsPort >= 3) C.setId_inPort(IdPort, 2, IdInput2); // FEITO
  // O parametro IdInput3 soh eh levado em conta se NumInputsPort>=4
  if (NumInputsPort >= 4) C.setId_inPort(IdPort, 3, IdInput3); // FEITO
...
void MainCircuito::slotModificarSaida(int IdSaida, int IdOrigemSaida)
{
  ...
  C.setIdOutput(IdSaida, IdOrigemSaida); // FEITO
...
void MainCircuito::redimensionaTabelas()
{
  ...
  int numInputs= C.getNumInputs(); // FEITO
  int numOutputs= C.getNumOutputs(); // FEITO
  int numPorts= C.getNumPorts(); // FEITO
...
void MainCircuito::showPort(unsigned i)
{
  ...
  bool indice_valido= C.validPort(i+1); // FEITO
  ...
  QString namePort= QString::fromStdString(C.getNamePort(i+1)); // FEITO;
  int numInputsPort= C.getNumInputsPort(i+1); // FEITO
  ...
  {
    idInputPort[j] = C.getId_inPort(i+1, j); // FEITO
  }
...
void MainCircuito::showOutput(unsigned i)
{
  ...
  // Provisoriamente, o teste eh sempre falso
  bool indice_valido= C.validIdOutput(i+1); // FEITO
  ...
  int idOutput= C.getIdOutput(i+1); // FEITO
...
void MainCircuito::on_actionLer_triggered()
{
  ...
  if (!fileName.isEmpty()) {
    ...
    // Provisoriamente, o teste eh sempre falso
    bool leitura_OK = C.ler(fileName.toStdString()); // FEITO
    if (!leitura_OK)
    {
...
void MainCircuito::on_actionSalvar_triggered()
{
  ...
  // Provisoriamente, o teste eh sempre falso
  bool circuito_valido = C.valid(); // FEITO
  ...
  if (!fileName.isEmpty()) {
    ...
    // Provisoriamente, o teste eh sempre falso
    bool escrita_OK = C.salvar(fileName.toStdString()); // FEITO
...
void MainCircuito::on_actionGerar_tabela_triggered()
{
  ...
  for (i=0; i<numCombinacoesEntrada; i++)
  {
    ...
    for (j=0; j<numOutputs; j++)
    {
      ...
      bool3S output = C.getOutput(j+1); // FEITO
...
void MainCircuito::on_tablePortas_activated(const QModelIndex &index)
{
  ...
  QString namePort = QString::fromStdString(C.getNamePort(idPort)); // FEITO
  int numInputsPort = C.getNumInputsPort(idPort); // FEITO
  ...
  {
    idInputPort[j] = C.getId_inPort(idPort, j); // FEITO
  }
...
void MainCircuito::on_tableSaidas_activated(const QModelIndex &index)
{
  ...
  // Provisoriamente, estah sendo inicializado com valor nulo
  int idOrigemSaida = C.getIdOutput(idSaida); // FEITO
