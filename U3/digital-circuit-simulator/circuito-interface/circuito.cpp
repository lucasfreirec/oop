...
Circuito::Circuito(const Circuito& C): Nin(C.Nin), id_out(C.id_out), out_circ(C.out_circ), ports(C.ports.size())
{
    for(int i=0; i < int(ports.size()); ++i) ports.at(i) = C.ports.at(i)->clone();
}
...
Circuito::Circuito(Circuito&& C): Nin(C.Nin), id_out(), out_circ(), ports()
{
    C.Nin = 0;
...
void Circuito::clear()
{
    ...
    for(int i=0; i < int(ports.size()); ++i) delete ports.at(i);
    ports.clear();
}
...
void Circuito::resize(int NI, int NO, int NP)
{
    ...
    id_out.resize(NO);
    for (int i=0; i<int(id_out.size()); ++i) id_out.at(i) = 0;

    out_circ.resize(NO);
    for (int i=0; i<int(out_circ.size()); ++i) out_circ.at(i) = bool3S::UNDEF;

    ports.resize(NP);
    for (int i=0; i<int(ports.size()); ++i) ports.at(i) = nullptr;
}
...
bool Circuito::ler(const std::string& arq)
{
    ...
    try
    {
        ...
        {
            myfile >> id >> c >> id_orig;
            if(!myfile.good() || id!=i+1 || c!=')' || !validIdOrig(id_orig)) throw 7;
...
bool Circuito::simular(const std::vector<bool3S>& in_circ)
{
    ...
    for (int i=0; i<getNumPorts(); ++i) ports.at(i)->setOutput(bool3S::UNDEF);
    ...
    do
    {
        ...
        for (int i=0; i<getNumPorts(); ++i)
        {
            ...
            {
                in_port.resize(ports.at(i)->getNumInputs());
    ...
    for(int j=0; j<getNumOutputs(); j++)
    {
        ...
        if (id>0)
        {
            out_circ.at(j) = ports.at(id-1)->getOutput();
        }

        else
        {
            out_circ.at(j) = in_circ.at(-id-1);
        }
    }
