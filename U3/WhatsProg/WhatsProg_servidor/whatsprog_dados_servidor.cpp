...
bool Usuario::ler(ifstream& arq)
{
    ...
    try
    {
        ...
        arq >> Login >> Senha >> lastId;

        if (!testarNomeUsuario(Login) || !testarSenha(Senha) || lastId<0) throw 2;
    }
    catch (int i)
    {
        return false;
...
void WhatsProgDadosServidor::enviarMensagem(iterMensagem iMsg, iterUsuario iDest)
{
    try
    {
        ...
        itU = find(LU.begin(), LU.end(), iMsg->getRemetente());

        if (itU->connected())
        {
            enviarConfirmacao(iMsg, itU);
        }
    }
    catch (int erro)
    {
...
bool WhatsProgDadosServidor::ler()
{
    ...
    try
    {
        ...
        arq >> prov_cabecalho >> NumUsuarios >> NumMensagens;

        if (prov_cabecalho != "DADOS_SERVIDOR") throw 2;
    ...
    {
        if (!prov_U.ler(arq)) return false;
    ...
    {
        if (!prov_M.ler(arq)) return false;
...
bool WhatsProgDadosServidor::salvar() const
{
    ...
    arq << "DADOS_SERVIDOR " << getNumUsuarios() << ' '
        << getNumMensagens() << endl;

    for (listUsuario::const_iterator i = LU.begin(); i != LU.end(); ++i)
    {
        if (!i->salvar(arq)) return false;
    }

    for (listMensagem::const_iterator j = LM.begin(); j != LM.end(); ++j)
    {
        if (!j->salvar(arq)) return false;
    }
